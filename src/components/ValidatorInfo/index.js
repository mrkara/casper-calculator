...
export default class ValidatorInfo extends Component {
  ...
  async tick() {
    ...
    var delegator_count = validator_data.delegators.length;
    let total_staked = 0;
    let delegator_pk = [];
    ...
    for (var j=0;j<delegator_count;j++){
    ...
    //console.log(delegator_chart_data);
    //this.setState({delegator_chart_data:delegator_chart_data})
    this.setState({
        ...
        <div className="container" style={{marginTop:"50px"}}>
            ...
            <div className="col-md-6 mb-1">
              <form action="#" className="p-3 bg-white">
                ...
                {this.state.validator_website !== "" && this.state.validator_website  ?
                  <a href = {this.state.validator_website} target='_blank'  >{this.state.validator_website}</a>
                :
                  "-"
                }</h3>
                <p> Commission Rate</p>
          ...
          <div className="row mb-5">
            <div className="col-md-12 mb-1 text-center">
              <form action="#" className="p-3 bg-white">
                <a href={'https://cspr.live/delegate-stake?validatorPublicKey='+this.state.public_key} className="btn btn-primary" target="_blank">DELEGATE CSPR TO THIS VALIDATOR</a>
              </form>
            </div>
          </div>
          <div className="row mb-5">
                    ...
                    <Tbody>
                      ...
                      {this.state.delegators_data ?
                        this.state.delegators_data.map((delegator, index) => (
                            <Tr key={index+1}>
                              <Td><a href={"#/account/"+delegator.address}>{utils.truncateStr(delegator.address,5)}</a></Td>
                              <Td>{utils.numberWithCommas(parseFloat(delegator.staked_amount).toFixed(3))}</Td>
                              <Td><a href={void(0)} className="btn btn-primary" onClick={()=>this.openModal(delegator.address)}>View Earnings</a></Td>
                            </Tr>
                        ))
                        :
                        null
